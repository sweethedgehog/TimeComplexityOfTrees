...
int genPriority() {
    std::uniform_int_distribution<> generator(-100000000, 100000000);
...
class CoolTreap {
    ...
    Node *removeRec(Node *&curr, T value) {
        if (curr == nullptr) {
            return nullptr;
        }
        if (curr->value == value) {
    ...
    std::pair<Node *, Node *> splitRec(Node *curr, T value) {
        if (curr == nullptr) {
    ...
    std::pair<Node *, Node *> split(Node *curr, T value) {
        std::stack<Node *> s1;
        std::stack<Node *> s2;
        Node *currPtr = curr;
